...
import { deleteProduct } from "../../../../StateManagement/Slices/ProductsSlices/ProductsSlices";
...
export const Delete = ({ id }) => {
  ...
  const Delete = async () => {
    try {
      ...
      const response = await axiosClient.delete(`api/items/${id}`);
      ...
      <Modal id={`deleteRequest${id}`}>
        <div className="flex flex-col gap-4 items-center bg-light-surface dark:bg-dark-surface px-8 py-6 rounded-xl  ">
          <div className="flex gap-4">
            <span className="text-body-large">تم حذف المنتج</span>
